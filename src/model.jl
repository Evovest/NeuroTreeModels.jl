...
function (m::NeuroTree)(x::AbstractMatrix{T}) where {T}
    ...
    # pred = dot_prod_agg(lw, m.p) ./ T(√size(m.p, 3))
    return pred
end
...
"""
function NeuroTree(; ins, outs, depth=4, ntrees=64, act=identity, init_scale=1e-1)
    nnodes = 2^depth - 1
    nleaves = 2^depth
    nt = NeuroTree(
        ...
        Float32.(randn(outs, nleaves, ntrees) .* init_scale), # p ./ √ntrees
        act
    )
    return nt
end
function NeuroTree((ins, outs)::Pair{<:Integer,<:Integer}; depth=4, ntrees=64, act=identity, init_scale=1e-1)
    nnodes = 2^depth - 1
    nleaves = 2^depth
    nt = NeuroTree(
        Float32.((rand(nnodes * ntrees, ins) .- 0.5) .* √(12 / ins)), # w
...
function StackTree((ins, outs)::Pair{<:Integer,<:Integer}; depth=4, ntrees=64, stack_size=2, hidden_size=8, act=identity, init_scale=1e-1)
    @assert stack_size == 1 || hidden_size >= outs
    trees = []
    for i in 1:stack_size
        if i == 1
            if i < stack_size
                tree = NeuroTree(ins => hidden_size; depth, ntrees, act, init_scale)
                push!(trees, tree)
            else
                tree = NeuroTree(ins => outs; depth, ntrees, act, init_scale)
                push!(trees, tree)
            end
        elseif i < stack_size
            tree = NeuroTree(hidden_size => hidden_size; depth, ntrees, act, init_scale)
            push!(trees, tree)
        else
            tree = NeuroTree(hidden_size => outs; depth, ntrees, act, init_scale)
            push!(trees, tree)
        end
    end
    m = StackTree(trees)
